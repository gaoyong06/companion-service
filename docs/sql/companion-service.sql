...
COMMENT ON COLUMN companion_conversation.status IS '会话状态：active 表示进行中，closed 表示已关闭';
COMMENT ON COLUMN companion_conversation.summary IS '会话摘要，关闭会话时由模型生成';
...
COMMENT ON COLUMN companion_message.user_id IS '所属用户唯一标识，冗余保存以支持用户范围查询和删除';
